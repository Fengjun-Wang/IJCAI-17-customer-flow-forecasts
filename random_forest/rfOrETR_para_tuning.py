...
if __name__=='__main__':
    ...
    getPeriod = -50
    print "loading training dataset from 1 to 2000"
    ...
    for predictors_type in ['withShopStatic']:
        ...
        for algorithm_name in ["rf", "etr"]:
            ...
            ReportFolder = os.path.join(HOME, "Dropbox", "dataset", "Analysis", "mimo_rf", "more_features_modified",
                                        algorithm_name)
                    ...
                    for n_estimators in [500]:
                        for min_samples_split in [2]:
                            for min_samples_leaf in [1]:
                                print "alg:%s,iter:%s,n_estimator:%s,mss:%s,msl:%s"%(algorithm_name,iter,n_estimators,min_samples_split,min_samples_leaf)
                                pickle_model_file_name = "%s_%s_iterLength_%s_n%s_minS%s_minL%s.csv" % (algorithm_name, predictors_type, iter, n_estimators, min_samples_split, min_samples_leaf)
                                ...
                                if algorithm_name == "etr":
                                    estimator_withParams = ETR(n_estimators=n_estimators, max_features="auto",min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, oob_score=True, n_jobs=-1, random_state = 2017)
                                rfOrETR_para_tuning(source=source, ReportFolder = ReportFolder, pickle_model_file_name = pickle_model_file_name, fgfs = fgfs, estimator_withParams = estimator_withParams, report_file = report_file, report_per_shop = '1.csv', predictors_type = predictors_type, predictors = predictors, target_variables = target_variables, estimator_output_length=iter, required_prediction_length=14, ifSavePickle = ifSavePickle)
