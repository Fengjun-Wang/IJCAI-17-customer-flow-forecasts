...
from tianchi_api.competition import CompetitionPredictionModel, IterativePredictionModel
from tianchi_api.features import *

def Model_for_competition(algorithm_name, pickle_model_file_name, fgfs, ReportFolder, source, predictors_type, predictors, target_variables, estimator_output_length = 14, required_prediction_length = 14, ifSaveModel = False, predict_mode = ["drop", "filled"], n_estimators = 300, min_samples_split =2, min_samples_leaf =1, iterOrCopy = "iterative", estimator_withParams = "l", xgb_param = "l", early_stopping_rounds = 50):
    ...
    for mode in predict_mode:
        ...
        x = CompetitionPredictionModel(estimator_withParams, estimator_output_length, required_prediction_length, fgfs, predictors, 5, True, mode)
        # x.predict_shop(shop_id = 1)
        x.do_competition(save_to_file = competition_result,start=1,end=2000)
                ...
                for n_estimators in [500]:
                    for min_samples_split in [2]:
                        for min_samples_leaf in [1]:
                            ...
                            Model_for_competition(algorithm_name = algorithm_name, estimator_withParams = estimator_withParams, pickle_model_file_name = "1.csv", fgfs = fgfs, ReportFolder = ReportFolder, source = source, predictors_type = predictors_type, predictors = predictors,target_variables = target_variables, estimator_output_length=estimator_output_length, required_prediction_length=required_prediction_length, ifSaveModel=False, predict_mode=["drop", "filled"],  n_estimators = n_estimators, min_samples_split = min_samples_split, min_samples_leaf = min_samples_leaf,iterOrCopy = iterOrCopy)
