...
def Model_for_competition(algorithm_name, pickle_model_file_name, fgfs, ReportFolder, source, target_variables, estimator_output_length = 14, required_prediction_length = 14, ifSaveModel = False, predict_mode = ["drop", "filled"], n_estimators = 300, min_samples_split =2, min_samples_leaf =1, iterOrCopy = "iterative", estimator_withParams = "l", xgb_param = "l", early_stopping_rounds = 50):
    ...
    if algorithm_name == "xgBoost":
        ...
        estimator_withParams.fit(X_train[predictors], y_train)
    ...
    for mode in predict_mode:
        # DesFolder = os.path.join(HOME, "Dropbox", "dataset", "Analysis", "competition", "Feature_for_competition")
        # des_csv_file = "features_for_competition_filled.csv"
        # des_csv_file = os.path.join(DesFolder, des_csv_file)
        # X_test = pd.read_csv(des_csv_file)
        #
        # y_pred = estimator_withParams.predict(X_test[predictors])
        # y_pred = np.round(y_pred).astype(int)
        # competition_result_file = "offlineGeneratedFeature_%s_comp_%s_%s_%s_iterLength_%smode_%s_tarLength_%s_iterOrC_%s.csv" % (
        #     algorithm_name, n_estimators, min_samples_split, min_samples_leaf, estimator_output_length, mode,
        #     required_prediction_length, iterOrCopy)
        # competition_result = os.path.join(ReportFolder, competition_result_file)
        # with open(competition_result, 'w') as fw:
        #     res = []
        #     for i in xrange(1, 2001):
        #         res.append([i])
        #         res[-1].extend(y_pred[i - 1])
        #     for r in res:
