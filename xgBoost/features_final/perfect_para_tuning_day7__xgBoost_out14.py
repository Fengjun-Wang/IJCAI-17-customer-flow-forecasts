...
def xgBoost_out14(source, day, predictors, predictors_type, ifGS = True,  target_variables = ['Tar_1', 'Tar_2'], ifCompetition = False, useTrainCV=True, cv_folds=5, early_stopping_rounds=50, X_test_comp = 'l'):
    ...
    for tar in  target_variables[day-1:day]:
        ...
        if ifGS:
            ...
            cv_params_1 = {'max_depth': [7,8,9], 'min_child_weight': [1,3,5]}
            ...
            cv_params_3 = { 'subsample': [0.9], 'colsample_bytree': [i/10.0 for i in range(7,10)]}
            ...
            if best_score_3 < best_score_2:
                best_params_3 = {'subsample': 0.8, 'colsample_bytree': 0.8}
            ...
            cv_params_5 = {'reg_alpha': [ 1, 100, 150]}
        ...
        if ifGS == False:
            ...
            xgb_param = {'reg_alpha':1, 'subsample': 0.9,  'learning_rate': 0.05, 'colsample_bytree': 0.7, 'max_depth': 7, 'gamma': 0.1, 'min_child_weight': 1,'seed': 0, 'objective': 'reg:linear'}
            dtrain = xgb.DMatrix(X_train.values, y_train[tar].values, feature_names=X_train.columns.values)
            deval = xgb.DMatrix(X_test.values, y_test[tar].values, feature_names=X_test.columns.values)
            watchlist = [(dtrain, 'train'), (deval, 'val')]
            xgtrain = xgb.DMatrix(source[predictors].values, label=source[tar].values,
            ...
            with open(report_file_confirm_iter, 'a+') as fw:
                fw.write(
                    "-------------------After choose all paras, test best rounds for a smaller learning rate, 0.05\:\n")
                fw.write(str(xgb_param))
                fw.write("\n")
                fw.write(str(cvresult.shape[0]))
...
if __name__=='__main__':
    ...
    for para in [1]:
                    ...
                    xgBoost_out14(source, day, predictors, predictors_type = predictors_type, target_variables = target_variables, X_test_comp = X_test_forCom, ifGS=False, ifCompetition=False, useTrainCV=True, cv_folds=3, early_stopping_rounds=50)
